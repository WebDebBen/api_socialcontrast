		...
		success: function(data){
			if (data["status"] == "success" ){
				if (id == "-1"){
					...
					table.row.add( ["<div class='posts_title'>" + tr_title + "</div>", "<div class='posts_content'>" + tr_content + "</div>", '<button class="btn btn-xs btn-sm btn-primary mr-6 edit-item" data-id="' + table_id + '"><i class="fa fa-edit"></i></button><button class="btn btn-xs btn-sm btn-secondary delete-item" data-id="'+ table_id + '"><i class="fa fa-trash"></i></button>']).draw( false );
				}else{
...
function load_data(data ){
	...
	for(var i = 0; i < data.length; i++ ){
		...
		td = $("<td>").appendTo(tr);
		$("<div>").addClass("posts_content").html(item[2]).appendTo(td);
