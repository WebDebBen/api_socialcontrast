<?php
    ...
    function handle_post_method(){
        ...
        $dir = "../../../" . PLUGIN_PATH . "/api/documentation/"; 
        if (!file_exists($dir )){
            mkdir($dir, 0700);
        }
    ...
    function generate_create_php($table_name ){
        ...
        $php .= "\t" . 'include_once("' . $table_name . '.php");' . "\n"; 
        ...
        $php .= "\t" . '$model = new ' . $table_class . '($db->conn );' . "\n";
    ...
    function _model_table_join_data(){
        $php = add_tab_endline('private function table_join_data($table, $field, $value ){');
            ...
            $php .= add_tab_endline('if($result){',2);
                $php .= add_tab_endline('while ($row = $result->fetch(PDO::FETCH_BOTH)){',3);
    ...
    function _model_add_or_update($columns ){
        ...
        $php .= add_tab_endline('}else{', 2);
            ...
            $php .= add_tab_endline('if ($result->rowCount() > 0 ){', 3);
    ...
    function _model_delete_php($columns ){
        ...
        $php .= add_tab_endline('if($stmt = $this->conn->query($query )){', 2);
            $php .= add_tab_endline('return ["validation"=> $validate, "result"=> true  ];', 3);
        $php .= add_tab_endline('}else{', 2);
            $php .= add_tab_endline('return ["validation"=> $validate, "result"=> $this->conn->errorInfo() ];', 3);
        $php .= add_tab_endline('}', 2);  
    ...
    function _model_update_php($columns ){
        ...
        $php .= 'if (strlen($join_query) ==0 ){';
			$php .='return ["validation"=> [], "result"=> "no date for updating"];}';

        $php .= add_tab_endline('if($stmt = $this->conn->query($query )){', 2);
            $php .= add_tab_endline('return ["validation"=> $validate, "result"=> true, "affected_rows"=> $stmt-rowCount()  ];', 3);
        $php .= add_tab_endline('}else{', 2);
            $php .= add_tab_endline('return ["validation"=> $validate, "result"=> $this->conn->errorInfo() ];', 3);
        $php .= add_tab_endline('}', 2);  
    ...
    function _model_init_insert($columns){
        ...
        $php .= '$str_pos = strpos($query, " set ,");';
		$php .= '$query = $str_pos ? substr($query, 0, $str_pos + 5) . substr($query, $str_pos + 7) : $query;';
        return $php;
    }
    ...
    function _model_init_update($columns){
        ...
        $php .= add_tab_endline('$join_query = $this->join_set_data($set_arr );', 2);
        $php .= add_tab_endline('$query .= $join_query;', 2);
        return $php;
    }
    ...
    function _model_read_php($columns ){
        ...
        $php .= add_tab_endline('$data = [];', 2);
        $php .= add_tab_endline('$result = $this->conn->query($query );', 2);
		$php .= add_tab_endline('$retrieve_total = $result->rowCount();', 2);
