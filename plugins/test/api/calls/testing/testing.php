<?php 
	...
	public function read($params ){
		...
		$description1 = isset($description1) ? $description1: "";
		...
		if ($description1 != ""){
			$query .= " and description1={$description1}";
		}
		if ($year != ""){
			$query .= " and year={$year}";
		}
		...
		if($result){
			while ($row = $result->fetch(PDO::FETCH_BOTH)){
				$row["admins"] = $this->table_join_data("admins", "id", $row["admin_id"]);
				array_push($data, $row );
			}
		}
	...
	public function create($params ){
		...
		$query .= $description1 ? ", description1='{$description1}'" : "";
		...
		$query .= $admin_id ? ", admin_id={$admin_id}" : "";
$str_pos = strpos($query, " set ,");$query = $str_pos ? substr($query, 0, $str_pos + 5) . substr($query, $str_pos + 7) : $query;		if($this->conn->query($query )){
	...
	public function update($params ){
		...
		array_push($set_arr, ["field"=> "description1", "value"=> $description1, "type"=> "text"]);
		array_push($set_arr, ["field"=> "year", "value"=> $year, "type"=> "int"]);
		...
		array_push($set_arr, ["field"=> "admin_id", "value"=> $admin_id, "type"=> "int"]);
