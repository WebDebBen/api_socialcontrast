<?php 
	...
	public function create($params ){
		...
		$ddd = isset($ddd) ? $ddd: "";
		if ($bbb == ""){
			$validate["bbb"] = "required";
		}
		if ($ccc == ""){
			$validate["ccc"] = "required";
		}
		if ($ddd == ""){
			$validate["ddd"] = "required";
		}
		if (count($validate ) > 0 ){
...
$str_pos = strpos($query, " set ,");$query = $str_pos ? substr($query, 0, $str_pos + 5) . substr($query, $str_pos + 7) : $query;		if($this->conn->query($query )){
	...
	public function update($params ){
		...
		$set_arr = [];
		array_push($set_arr, ["field"=> "bbb", "value"=> $bbb, "type"=> "varchar"]);
		array_push($set_arr, ["field"=> "ccc", "value"=> $ccc, "type"=> "varchar"]);
		array_push($set_arr, ["field"=> "ddd", "value"=> $ddd, "type"=> "varchar"]);
